from langchain_functions.agents.query_rag.vector_store.upload_data_to_db import (
    upload_to_db,
)
...
async def add_file(path: str):
    ...
    if os.path.join(path).endswith(".pdf"):
        from langchain_functions.agents.query_rag.vector_store.loaders import pdf_pages

        await upload_to_db(pdf_pages(path))

    if os.path.join(path).endswith(".html"):
        from langchain_functions.agents.query_rag.vector_store.loaders import html_pages

        await upload_to_db(html_pages(path))

    if os.path.join(path).endswith(".json"):
        from langchain_functions.agents.query_rag.vector_store.loaders import json_pages

        await upload_to_db(json_pages(path))

    if os.path.join(path).endswith(".jpg") or os.path.join(path).endswith(".png"):
        from langchain_functions.agents.query_rag.vector_store.loaders import (
            image_pages,
        )
    ...
    if os.path.join(path).endswith(".txt"):
        from langchain_functions.agents.query_rag.vector_store.loaders import text_pages
