...
class Response(BaseModel):
    ...
    source_desc = "List of URL's, snippets, and titles which provide sources to help answer the question. \
    ...
    sources: List = Field(description=source_desc)

# helper functions...
def outputParser(output): # parse the output
    # If no function was invoked, return to user
    if "function_call" not in output.additional_kwargs:
        return AgentFinish(return_values={"storedAnswer": output.content}, log=output.content) # so its getting stuck here
...
def searchOrNot(userIn: str) -> List[dict]: 
    ...
    return agent_executor.invoke({"userQuery": userIn})
